...
void backword(unsigned int leftSpeed, unsigned int rightSpeed)
{
...
void forword(unsigned int leftSpeed, unsigned int rightSpeed)
{
...
void turn_STOP()
{
    ...
    //digitalWrite(moR_cw, LOW);
    //digitalWrite(moR_ccw, LOW);
    analogWrite(pin_pwm_R, LOW);
    analogWrite(pin_pwm_L, LOW);
}
...
void setup()
{
    ...
    { 
        ...
        if (i < 12 || i >= 38) // turn to the left and right to expose the sensors to the brightest and darkest readings that may be encountered
